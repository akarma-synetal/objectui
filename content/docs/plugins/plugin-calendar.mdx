...
`MonthView` and `WeekView` both support drag-to-reschedule out of the box:
...
- **Resize an event** (MonthView only) — grab the small right-edge handle on the last day of a multi-day pill and drop it on a different day to extend or shrink the end date. The start date is preserved; drops earlier than the start are ignored.
...
Clicking the empty area of any day cell opens a quick-create dialog
pre-filled with the clicked date. Type a title and press <kbd>Enter</kbd>
(or click **Create**) to persist a new record via
