...
export const SAMPLES: Record<string, Record<string, unknown>> = {
  ...
  flow: {
    ...
    nodes: [
      { id: 'start', type: 'start', label: 'Schedule (daily)', config: { triggerType: 'schedule', schedule: '0 9 * * *', condition: '' } },
      { id: 'find', type: 'get_record', label: 'Find expiring contracts', config: { objectName: 'contract', filters: { status: 'active' }, outputVariable: 'contracts' } },
      { id: 'check', type: 'decision', label: 'Within reminder window?', config: { expression: 'daysToExpiry <= daysBefore' } },
      ...
      { id: 'skip', type: 'update_record', label: 'Mark as not due', config: { objectName: 'contract', recordId: 'contract.id', fields: { reminded: false } } },
